...
//this is a temp function meant to resolve lape leaks.
function Wrapper(mat: T2DIntArray; angle:Double): T2DIntArray;
var tmp1,tmp2,tmp3: T2DIntArray;
begin
  tmp1 := w_imRotate(mat, angle, False,False);
  tmp2 := w_GetArea(tmp1, WMM_INNER.x1,WMM_INNER.y1,WMM_INNER.x2,WMM_INNER.y2);
end;

function TRSPosFinder.Correlate(sub,large:T2DIntArray; offset:Double=0): T2DFeatPointArray;
var
  tmp1,tmp2,tmp3:T2DIntArray;
  angle:Int32;
  test:T2DIntArray;
begin
  for angle:=-20 to 20 with 2 do
  begin
    tmp1 := w_imRotate(sub, radians(angle)+offset, False,False);
    tmp2 := w_GetArea(tmp1, WMM_INNER.x1,WMM_INNER.y1,WMM_INNER.x2,WMM_INNER.y2);
    tmp3 := w_imSample(tmp2, self.scanRatio);
    SetLength(Result, length(Result)+1);
    Result[high(Result)] := Self.FastMatchTemplate(large, tmp3, angle);

    SetLength(tmp1,0);       //leakfix - Simba 1.2rc6
    SetLength(tmp2,0);       //
    SetLength(tmp3,0);       //
    (*
...
var
  ...
  BMP: Integer;
  MM,tmp1,tmp2,world: T2DIntArray;
  best:TFeaturePoint;
  compRad:Double = 0;
begin
